    ...
    for project in projects:
        ...
        for i in range(num_properties):
            ...
            property_doc = {
                ...
                'unit_number': f"{chr(65 + random.randint(0, 10))}-{random.randint(101, 999)}",
                'type_id': prop_type_cat['id'],
                'status_id': status_cat['id'] if status_cat else None,
                'floor': random.randint(0, 25),
                'tower': random.choice(['A', 'B', 'C', 'D']),
                'facing': random.choice(['North', 'South', 'East', 'West', 'North-East', 'South-West']),
                'area_sqft': area,
                'area_sqm': round(area * 0.092903, 2),
                'base_price': base_price,
                'final_price': base_price,
                'price_per_sqft': price_per_sqft,
                'description': f"Spacious {prop_type} with modern amenities",
                'specifications': {
                    'bedrooms': random.randint(1, 4) if 'BHK' in prop_type else 0,
