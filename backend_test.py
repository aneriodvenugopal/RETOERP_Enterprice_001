...
def test_tenant_admin_properties(auth_token):
    ...
    try:
        headers = {"Authorization": f"Bearer {auth_token}"}
        response = requests.get(f"{API_BASE}/properties/", headers=headers, timeout=10)  # Added trailing slash
...
def test_tenant_admin_leads(auth_token):
    ...
    try:
        headers = {"Authorization": f"Bearer {auth_token}"}
        response = requests.get(f"{API_BASE}/leads/", headers=headers, timeout=10)  # Added trailing slash
